...
class ProfileNationality {
    private $db;
    private $table_name = "profile_nationality";
    public $id;
    public $name;
    public $namearabic;
    public $createddate;
 ...
 public function create(){
        ...
        if( $this->db->affected_rows > 0 ) {
            echo json_encode( array(
                'status' => true,
                'msg' => 'Profile Nationality Created Successfully',
            ) );
        } else {
            echo json_encode( array(
                'status' => false,
...
public function getlocation() {
    ...
    public function update(){
        ...
        if( $this->db->affected_rows > 0 ) {
            echo json_encode( array(
                'status' => true,
                'msg' => 'Profile Nationality Updated Successfully',
            ) );
        } else {
            echo json_encode( array(
                'status' => false,
    ...
    public function deleteById( $cid ){
        ...
        if( $this->db->affected_rows > 0 ) {
            echo json_encode( array(
                'status' => true,
                'msg' => 'Profile Nationality Deleted Successfully',
            ) );
        } else {
            echo json_encode( array(
                'status' => false,
                'msg' => 'Profile Nationality is not deleted or doesn\'t exist in system',
            ) );
        }
    }
