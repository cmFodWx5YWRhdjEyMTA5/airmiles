...
class Category {
    private $db;
    private $table_name = "appcategory";
    public $id;
    public $image;
    public $name;
    public $order;
 ...
 public function create(){
        $sql = "insert into " . $this->table_name. "(name,namearabic,image,orderby)VALUES('" . $this->name . "','" . $this->namearabic . "','" . $this->image . "','" . $this->order . "')";
    ...
    public function getClientsDataTable( $data ) {
        ...
        while ($row = mysqli_fetch_assoc($query)) {
            ...
            $json[$k]['image'] = "<img src='../img/appcategory/".$row['image']."' width='150px' /> ";
...
$json[$k]['actions'] = '<button type="button" data-id = "'.$row['id'].'"  class="edit_Clients btn ink-reaction btn-floating-action btn-xs btn-info" data-toggle="tooltip" data-placement="top"  data-original-title="Edit" title="Edit">
  ...
  public function getcategory() {
    ...
    public function update(){
        $sql = "UPDATE " . $this->table_name . " set `name`='" . $this->name . "',`namearabic`='" . $this->namearabic . "',`image`='" . $this->image . "',`orderby`='" . $this->order . "' where id=" . $this->id;
