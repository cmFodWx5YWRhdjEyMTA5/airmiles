jQuery(document).ready(function ($) {
    var clients_list_table = jQuery('#clients_list_table').DataTable({
        ...
        "order": [[1, 'asc']],
        "sAjaxSource": "process/clientsprocess.php",
...
jQuery(document).on('click','.add_client',function(e){
    ...
    jQuery(document).on('click','.edit_Clients',function(e){
        ...
        jQuery.ajax({
            url: 'process/clientsprocess.php',
            type: 'POST',
            dataType: 'json',
            ...
            success: function(response) {
                ...
                jQuery('#EditreferralsModal #subtitle').val(response.subtitle);
                jQuery('#EditreferralsModal #text').val(response.text);
                jQuery('#EditreferralsModal #email').val(response.email);
                jQuery('#EditreferralsModal #imgsrc').val(response.logo);
                
                document.getElementById("logoimageuploaded").src="../img/"+response.logo;
    ...
    jQuery('#add_client_form').submit(function(event){
        ...
        jQuery.ajax({
            url: 'process/clientsprocess.php?action=create_Clients',
            type: 'POST',
            dataType: 'json',
    ...
    jQuery('#edit_referrals_form').submit(function(event){
       ...
       jQuery.ajax({
            url: 'process/clientsprocess.php?action=update_Clients',
            type: 'POST',
            dataType: 'json',
            data: new FormData(this),     
            contentType: false,
