 <?php
require_once '../classes/classHomeslider.php';

error_reporting(0);
ini_set('display_errors', 1);

if ($_REQUEST['action'] == 'get_clients') {
    $ClientsObj       = new Homeslider($db);
    $json['aaData']   = array();
...
if ($_REQUEST['action'] == 'create_Clients') {
    ...
    $ClientsObj->url      = !empty($_POST['url']) ? $_POST['url'] : false;
    $ClientsObj->language = !empty($_POST['language']) ? $_POST['language'] : false;
    if ($_FILES['logoimage']['name'] != "") {
        ...
        move_uploaded_file($file_tmp, dirname(dirname(dirname(__FILE__))) . '/img/homeslider/' . $newfilename_1);
